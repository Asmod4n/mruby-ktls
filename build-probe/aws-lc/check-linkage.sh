...
CRYPTO_LIB_NAME="libcrypto.so"
SSL_LIB_NAME="libssl.so"
ABI_VERSION="0"
...
case "${LIBRARY_TYPE}" in
  "crypto")
    if ! (ldd "${FILE_TO_CHECK}" | grep -q "${INSTALL_LIBDIR}/${CRYPTO_LIB_NAME}"); then
    ...
    ;;
  "ssl")
    if ! (ldd "${FILE_TO_CHECK}" | grep -q "${INSTALL_LIBDIR}/${SSL_LIB_NAME}"); then
